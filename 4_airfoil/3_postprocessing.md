...
We can clearly see the flow around the airfoil with the following key flow features: (1) Stagnation point at the airfoil leading edge, (2) fluid acceleration at the upper and lower surface (symmetric), (3) a thin, symmetric wake downstream, and (4) a symmetric pressure distribution about the chord line.
...
Reference data for pressure coefficient is provided in the `experimental_data` folder for an angle of attack of 0 degrees called `pressure_coefficient.csv`. The file is formatted in CSV file format and can be imported to ParaView for visualization. Click on **File** $$\rightarrow$$ **Open...** to open the files and click **Apply**. With the following steps, we can add the reference data to the diagram:
...
Before plotting the lift and drag coefficients against the experimental data, the results have to be extracted into a separate `results.csv` file with the following format:

```
"alpha","Cd","Cl"
0,1.123571e-02,-2.184655e-04
4,...,...
8,...,...
```
